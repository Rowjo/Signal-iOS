    ...
    (OWSDisappearingMessagesConfiguration *)disappearingMessagesConfiguration
{
    ...
    if (!self.navigationBarTitleView) {
        ...
        self.navigationBarSubtitleLabel.text = NSLocalizedString(
            @"MESSAGES_VIEW_TITLE_SUBTITLE", @"The subtitle for the messages view title indicates that the title can "
                                             @"be tapped to access settings for this conversation.");
                      ...
                      date:(NSDate *)date
{
    ...
    if ([text lengthOfBytesUsingEncoding:NSUTF8StringEncoding] > kMaxTextMessageSize) {
        UIAlertController *controller = [UIAlertController
            alertControllerWithTitle:NSLocalizedString(@"CONVERSATION_VIEW_TEXT_MESSAGE_TOO_LARGE_ALERT_TITLE",
    ...
    attributedTextForCellBottomLabelAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (message.messageType == TSOutgoingMessageAdapter) {
        ...
        if (outgoingMessage.messageState == TSOutgoingMessageStateUnsent) {
            ...
            if (nextMessage && nextMessage.messageState != TSOutgoingMessageStateSent
...
- (void)showConversationSettings
{
    ...
    OWSConversationSettingsTableViewController *settingsVC =
        [[UIStoryboard storyboardWithName:AppDelegateStoryboardMain bundle:NULL]
        ...
        [[PHImageManager defaultManager]
            requestImageDataForAsset:asset
                       ...
                       resultHandler:^(NSData *_Nullable imageData,
                           ...
                           SignalAttachment *attachment =
                               [SignalAttachment imageAttachmentWithData:imageData dataUTI:dataUTI];
                           [self dismissViewControllerAnimated:YES
                                                    completion:^{
                                                        OWSAssert([NSThread isMainThread]);
                                                        if (!attachment || [attachment hasError]) {
...
- (void)sendQualityAdjustedAttachmentForVideo:(NSURL *)movieURL
{
    ...
    [exportSession exportAsynchronouslyWithCompletionHandler:^{
        ...
        SignalAttachment *attachment =
            [SignalAttachment videoAttachmentWithData:videoData dataUTI:(NSString *)kUTTypeMPEG4];
        if (!attachment || [attachment hasError]) {
            DDLogWarn(@"%@ %s Invalid attachment: %@.",
                self.tag,
                __PRETTY_FUNCTION__,
...
- (void)yapDatabaseModified:(NSNotification *)notification
{
    ...
    BOOL wasAtBottom
        = (self.collectionView.contentOffset.y + self.collectionView.bounds.size.height + kIsAtBottomTolerancePts
...
- (void)audioRecorderDidFinishRecording:(AVAudioRecorder *)recorder successfully:(BOOL)flag
{
    if (flag) {
        ...
        SignalAttachment *attachment =
            [SignalAttachment audioAttachmentWithData:audioData dataUTI:(NSString *)kUTTypeMPEG4Audio];
        if (!attachment || [attachment hasError]) {
            DDLogWarn(@"%@ %s Invalid attachment: %@.",
                self.tag,
                __PRETTY_FUNCTION__,
