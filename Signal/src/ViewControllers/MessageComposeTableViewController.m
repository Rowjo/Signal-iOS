...
- (void)createLoadingAndBackgroundViews
{
    ...
    [inviteButton addTarget:self action:@selector(presentInviteFlow) forControlEvents:UIControlEventTouchUpInside];
    [inviteButton setTitleColor:[UIColor ows_materialBlueColor] forState:UIControlStateNormal];
    ...
    [searchByPhoneNumberButton autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:inviteButton withOffset:20];
...
{
    self.isNoContactsViewVisible
        = ([self.contacts count] == 0 && ![[Environment preferences] hasDeclinedNoContactsView]);
}
...
{
    return (self.isNoContactsViewVisible ? 0 : MessageComposeTableViewControllerSection_Count);
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == MessageComposeTableViewControllerSectionInviteNonContactConversation) {
        if (indexPath.row < 0 || indexPath.row >= (NSInteger)self.searchPhoneNumberWithAccounts.count) {
            OWSAssert(0);
        }
        ...
        conversationForNonContactCell.textLabel.text =
            [NSString stringWithFormat:NSLocalizedString(@"NEW_CONVERSATION_FOR_NON_CONTACT_FORMAT",
        ...
        if (indexPath.row < 0 || indexPath.row >= (NSInteger)self.searchPhoneNumbers.count) {
            OWSAssert(0);
        }
        ...
        inviteViaSMSCell.textLabel.text =
            [NSString stringWithFormat:NSLocalizedString(@"SEND_INVITE_VIA_SMS_BUTTON_FORMAT",
                                           @"Text for button to send a Signal invite via SMS. %@ is placeholder for "
                                           @"the receipient's phone number."),
                      phoneNumber];
        ...
        OWSAssert(indexPath.section == MessageComposeTableViewControllerSectionContacts)
            ...
            = (ContactTableViewCell *)[tableView
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == MessageComposeTableViewControllerSectionInviteNonContactConversation) {
        ...
        OWSAssert(indexPath.section == MessageComposeTableViewControllerSectionContacts)
            ...
            NSString *identifier
            = [[[self contactForIndexPath:indexPath] textSecureIdentifiers] firstObject];
